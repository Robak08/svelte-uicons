...
const component = (icon) =>
  ...
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} fill="currentColor" viewBox="0 0 24 24" class="evaicon evaicon-${
		icon.name
	} {customClass}">${evaicon[icon.name]}</svg>
  `;
// export let strokeWidth = 0;
// stroke="currentColor" stroke-width="{strokeWidth}" stroke-linecap="round" stroke-linejoin="round"

const icons = Object.keys(evaicon).map((name) => ({
	name,
